"""
Direct Email Integration Fix V9
...
4. Completely disables CSRF validation in development mode for easier testing
"""
...
def add_direct_email_integration_routes():
    ...
    try:
        ...
        # IMPORTANT: Force CSRF validation to be disabled for ALL environments
        # This is a critical fix to ensure email integration works reliably
        csrf_enabled = False
        ...
        # Add compatibility endpoint for the old URL pattern for email connect
        @app.route('/api/integrations/email/connect', methods=['POST', 'OPTIONS'])
        def direct_email_connect_v1():
            """Legacy endpoint for connecting to email integration - v1 version for compatibility"""
            # Handle CORS preflight request
            if request.method == 'OPTIONS':
                response = jsonify({'status': 'success'})
                response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization')
            ...
            logger.info("Legacy email connect endpoint called, redirecting to v2")
            return direct_email_connect_v2()
            
        # Add compatibility endpoint for the old URL pattern for email disconnect
        @app.route('/api/integrations/email/disconnect', methods=['POST', 'OPTIONS'])
        def direct_email_disconnect_v1():
            """Legacy endpoint for disconnecting from email integration - v1 version for compatibility"""
            # Handle CORS preflight request
            if request.method == 'OPTIONS':
                response = jsonify({'status': 'success'})
                response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization')
            ...
            logger.info("Legacy email disconnect endpoint called, redirecting to v2")
            return direct_email_disconnect_v2()
