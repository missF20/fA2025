"""
Add Analytics Endpoints Directly to Main App

This script adds direct endpoints to handle analytics operations
...
def add_analytics_endpoints():
    """
    Add analytics endpoints directly to main.py 
    This ensures they're available regardless of blueprint registration
    """
    try:
        app_instance = app.app
        
        @app_instance.route('/api/analytics/test', methods=['GET'])
        def direct_analytics_test():
            """Test endpoint for analytics API that doesn't require authentication"""
            return jsonify({
                'success': True,
                'message': 'Analytics API is working (direct route)',
                'version': '1.0.0'
            })
            
        @app_instance.route('/api/analytics/data', methods=['GET'])
        @token_required
        @rate_limit(limit=30, per=60)
        def direct_analytics_data(user=None):
            """
        ...
        logger.error(f"Error adding direct analytics endpoints: {str(e)}")
        return False

if __name__ == "__main__":
    add_analytics_endpoints()