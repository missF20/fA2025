"""
Dana AI Platform - Zendesk Integration (Standardized)

This module provides standardized API routes for connecting to and interacting with Zendesk.
"""
...
def connect_integration():
    ...
    try:
        ...
        # Connect to Zendesk using the existing function
        result, status_code = connect_zendesk(data)
        
        if not result.get('success', False):
            return error_response(result.get('message', 'Failed to connect to Zendesk'), status_code)
        ...
        return success_response(
            message="Zendesk integration connected successfully",
            data={
                'integration_id': db_result['integration_id'],
                'connection_data': result.get('connection_data', {})
            }
        )
...
def integration_status():
    ...
    try:
        ...
        # Standard response structure with Zendesk-specific fields
        return success_response(
            data={
                'status': integration['status'],
                'configured': True,
                'last_updated': integration['date_updated'],
                'subdomain': integration['config'].get('subdomain', 'Unknown'),
                'email': integration['config'].get('email', 'Unknown')
            }
        )
...
def sync_integration():
    ...
    try:
        ...
        if not integration:
            return error_response("No active Zendesk integration found", 404)
            
        if integration['status'] != 'active':
            return error_response("Zendesk integration is not active", 400)
        
        # Call the existing sync function
        result = sync_zendesk(integration['id'], integration['config'])
        
        if not result.get('success', False):
            return error_response(result.get('message', 'Failed to sync with Zendesk'), 500)
            
        # Return standard success response
        return success_response(
            message="Zendesk sync initiated successfully",
            data=result.get('sync_status', {})
        )
