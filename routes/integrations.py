...
def get_integrations():
    ...
    try:
        ...
        integration_types = [
            integration.value for integration in IntegrationType
        ]
...
@integrations_bp.route('/integrations/schema/<integration_type>',
                       methods=['GET'])
@require_auth
    ...
    try:
        ...
        if not schema:
            return jsonify({
                ...
                False,
                "message":
                f"No schema available for integration type: {integration_type}"
            }), 404
...
@integrations_bp.route('/integrations/user/<user_id>/<integration_type>',
                       methods=['GET'])
@require_auth
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                ...
                False,
                "message":
                f"Invalid integration type: {data['integration_type']}"
            }), 400
        ...
        try:
            ...
            logger.warning(
                f"Could not send socket notification: {str(socket_error)}")
...
def update_integration(user_id, integration_type):
    ...
    try:
        ...
        if integration_key not in integration_status_cache:
            ...
            if 'config' in data:
                integration_status_cache[integration_key]['config'] = data[
                    'config']
            if 'status' in data:
                try:
        ...
        try:
            from socket_server import notify_integration_status_update
            notify_integration_status_update(
                integration_status_cache[integration_key], user_id)
        except Exception as socket_error:
            logger.warning(
                f"Could not send socket notification: {str(socket_error)}")
...
def delete_integration(user_id, integration_type):
    ...
    try:
        ...
        return jsonify({
            ...
            True,
            "message":
            f"Integration {integration_type} deleted successfully"
        }), 200
...
@integrations_bp.route('/integrations/user/<user_id>/<integration_type>/test',
                       methods=['POST'])
@require_auth
    ...
    try:
        ...
        if not verification.get('valid', False):
            return jsonify({
                "success": False,
                "message":
                f"Failed to connect to Slack API: {verification.get('message', 'Unknown error')}",
                "test_results": {
                    "connected": False,
        ...
        if not post_result.get('success', False):
            return jsonify({
                "success": False,
                "message":
                "Connected to Slack API but failed to send test message",
                "test_results": {
                    "connected": True,
                    "message_sent": False,
        ...
        return jsonify({
            "success": True,
            "message":
            "Successfully connected to Slack API and sent test message",
            "test_results": {
                "connected": True,
                "message_sent": True,
                "details": {
...
async def test_email_integration(config):
    ...
    try:
        ...
        if missing_fields:
            return jsonify({
                ...
                "test_results": {
                    ...
                    False,
                    "details":
                    f"Please provide these fields in your configuration: {', '.join(missing_fields)}"
                }
            }), 400
        ...
        result = await send_email(to_email=test_recipient,
        ...
        if not result:
            return jsonify({
                ...
                "test_results": {
                    ...
                    False,
                    "details":
                    "Unable to send email - check your SMTP configuration"
                }
            }), 500
...
async def test_postgresql_integration(config):
    ...
    try:
        from automation.integrations.database.postgresql import (execute_query,
                                                                 list_tables,
                                                                 list_databases
                                                                 )
        ...
        if not result:
            return jsonify({
                ...
                "test_results": {
                    ...
                    False,
                    "details":
                    "Unable to execute test query - check your database credentials"
                }
            }), 500
...
async def test_google_analytics_integration(config):
    ...
    return jsonify({
        ...
        "test_results": {
            ...
            False,
            "details":
            "Google Analytics integration test not fully implemented yet"
        }
    }), 200
...
@integrations_bp.route(
    '/admin/integrations/<user_id>/<integration_type>/status', methods=['PUT'])
@require_admin
...
def get_integration_status_dashboard():
    ...
    try:
        ...
        existing_types = [
            i['integration_type'] for i in dashboard_data["integrations"]
        ]
        ...
        dashboard_data["integrations"].sort(
            key=lambda x: x['integration_type'])

        # Add summary stats
        total = len(dashboard_data["integrations"])
        active = sum(1 for i in dashboard_data["integrations"]
                     if i['status'] == 'active')
        error = sum(1 for i in dashboard_data["integrations"]
                    if i['status'] == 'error')
        pending = sum(1 for i in dashboard_data["integrations"]
                      if i['status'] == 'pending')
        not_configured = sum(1 for i in dashboard_data["integrations"]
...
@integrations_bp.route('/integrations/status/<integration_type>',
                       methods=['GET'])
@require_auth
